...
/*function findRelatedWords(initialWord, nestedArray) {
    const relatedWords = [];
    ...
    function searchWords(array) {
      for (let i = 0; i < array.length; i++) {
        ...
        if (Array.isArray(item)) {
          searchWords(item);
        } else if (typeof item === 'string' && item.startsWith(initialWord)) {
          relatedWords.push(item);
        }
      }
    }
  
    searchWords(nestedArray);
  ...
  const initialWord = 'a';
  const relatedWords = findRelatedWords(initialWord, nestedData)*/
  ...
  function data(result, word) {
    ...
    function searchWords(array) {
      for (let i = 0; i < array.length; i++) {
        ...
        if (Array.isArray(item)) {
          searchWords(item);
        } else if (typeof item === 'string' && item.startsWith(word)) {
          relatedWords.push(item);
        }
      }
    }
